...
import type {
	...
	HighLevelOauthConfig,
	PrivateIntegrationConfig,
} from './config'
import type { HighLevelClientInterface } from './interface'

/*
This client implements the interface generated by the `generate-client` script.

As a result, the client will be in sync with the current state of the API. When new endpoints are added, they will be added to the interface and the client will then show type errors if they are not implemented.
*/
 ...
 *
 * Can also use private integration tokens in place of OAuth2 tokens.
 *
 * @example
 * ```ts
...
export class HighLevelClient<
	...
	TAuth extends BaseOauthClient | OauthClient<T> = BaseOauthClient,
> implements HighLevelClientInterface<T, TAuth>
{
	/**
	 ...
	 * ```ts
	 * const client = new HighLevelClient()
	 * const accessToken = client.oauth.GET()
	 * ```
	 */
	oauth: TAuth
	...
	constructor(
		...
		/**
		 * Base OAuth endpoint. For pre-configured OAuth use `createHighLevelClient`.
		 */
		this.oauth = createClient<Oauth.paths>(this._clientConfig) as TAuth
...
> extends HighLevelClient<T, TOAuth> {
	...
	privateToken: string
	/**
	 * Scopes for the private integration
	 */
	scopes: ScopeLiterals<T>[]
	constructor(config: {
		/**
		 * Private integration configuration. Only requires a `privateToken` and `scopes`.
		 *
		 * You can obtain the privateToken from your app's dashboard, or on the Settings page within the client-facing HighLevel app.
		 *
		 * @see https://help.gohighlevel.com/support/solutions/articles/155000003054-private-integrations-everything-you-need-to-know
		 */
		...
		const baseUrl =
			config.clientConfig?.baseUrl ??
			config.integrationConfig.baseUrl ??
			DEFAULT_BASE_URL

		this.privateToken = config.integrationConfig.privateToken
		this.scopes = config.integrationConfig.scopes
	}
}
...
export function createHighLevelClient<
	T extends AccessType,
	TAuthClient extends OauthClient<T> | BaseOauthClient = OauthClient<T>,
>(config: {
	oauthConfig?: HighLevelOauthConfig<T>
	clientConfig?: HighLevelClientConfig
...
export function createHighLevelClient<
	T extends AccessType,
	TAuthClient extends OauthClient<T> | BaseOauthClient = BaseOauthClient,
>(config: {
	integrationClient?: PrivateIntegrationConfig<T>
	clientConfig?: HighLevelClientConfig
	oauthConfig?: never
}): HighLevelClient<T, BaseOauthClient>
export function createHighLevelClient<
	T extends AccessType,
	TAuthClient extends OauthClient<T> | BaseOauthClient = BaseOauthClient,
>(config: {
	 ...
	 */
	clientConfig?: HighLevelClientConfig
}): HighLevelClient<T, TAuthClient> {
	if (!config.oauthConfig && !config.integrationConfig) {
		throw new Error(
			'Either oauthConfig or privateIntegrationConfig is required',
		)
	}
	if (config.oauthConfig) {
		return new HighLevelClientWithOAuth<T, TAuthClient>({
			clientConfig: config.clientConfig,
			oauthConfig: config.oauthConfig,
		})
	}
	if (config.integrationConfig) {
		return new HighLevelIntegrationClient<T, TAuthClient>({
			clientConfig: config.clientConfig,

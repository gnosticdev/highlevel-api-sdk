...
import { createHighLevelClient, type HighLevelConfig } from '../client/sdk'
...
app.get('/locations', async (c) => {
	...
	if (!locationId)
		throw new Error('Need a location ID to get locations installed!')
	coolConsole.obj({ locationId, token: !!accessToken })
...
const allRoutes = app.routes
	.filter((r) => !r.path.includes('*') && !r.path.match(/^\/auth(\/)?/))
	.map((r) => r.path)
